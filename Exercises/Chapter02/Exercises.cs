...
namespace Exercises.Chapter2
{
   ...
   {
        public static string Run(Func<double> getHeight, Func<double> getWeight) => RateWeight(getHeight(), getWeight());

        public static string RateWeight(double heightInM, double weightInKg) => RateBmi(CalculateBmi(heightInM, weightInKg));
        private static double CalculateBmi(double heightInM, double weightInKg) => weightInKg / (heightInM * heightInM);
        private static string RateBmi(double bmi) => bmi < 18.5 ? "underweight" : bmi >= 25 ? "overweight" : "healthy weight";
   }
}   
